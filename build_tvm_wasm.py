...
def build_module(opts):
    ...
    target_host = "llvm -mtriple=wasm32-unknown-unknown"  #
    if opts.mode == "webgpu":
        target_device = "webgpu -model=1080ti"
    else:
        target_device = "llvm -mtriple=wasm32-unknown-unknown"  #
...
def prepare_data(opts):
    ...
    shutil.copyfile(
        libinfo.find_lib_path("tvmjs_runtime.js")[0],
        # "/tvm/web/dist/wasm/tvmjs_runtime.js",
    ...
    shutil.copyfile(
        os.path.join(curr_dir, "index.html"), os.path.join(build_dir, "index.html")
    )
